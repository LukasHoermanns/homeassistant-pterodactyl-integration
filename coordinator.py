...
class PterodactylDataCoordinator(DataUpdateCoordinator):
    ...
    async def _calculate_scan_interval(self, scan_interval):
        ...
        if (scan_interval < min_interval):
            _LOGGER.warning(f"Scan Interval to Low. Rate Limit is 240/min. Every Server need one Api call, so minimum interval is {min_interval} beacuse your server count is {self.game_servers.count} ")
            return min_interval
    ...
    async def create_server(self) -> List[GameServer]:
        self.game_servers = []
        data = await self.apiHandler.getAllServers()
